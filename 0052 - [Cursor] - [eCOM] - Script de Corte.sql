create or replace PROCEDURE pedido_tormes_02
AS
BEGIN
...
-- 22/07/2020
    -- Atualização DBO.t_order
    -- Geração de NF
    -- Fechamento de Tarefas
-- 26/07/2020 - SIMOM
	-- Substituiu Exit por Continue no lopp do corte
	-- Incluida validacao de status do endereco e pick area no bloco de estoque disponivel
	-- Removida a validacao que se exitia alguma transacao de picking - alguns picking abrem em duas
    -- Incluido update created_nf = Y para notas geradas
-- 27/08/2020 - Gustavo Félix.
	-- Incluído a rastreabilidade de Corte
    -- Se o pedido já foi iniciado a separação, não será mais cortado pelo Script
...
CURSOR c_main IS
 SELECT
    ...
    SELECT
        ...
        , log.wh_id
        , CASE WHEN (SELECT COUNT(DISTINCT ord.item_number) FROM DBO.t_order_detail ord
        ...
        FROM dbo.t_exception_log LOG
            left join DBO.t_pick_detail pkd
...
CURSOR c_CloseTasks IS
    SELECT DISTINCT pkd.work_q_id AS wk_id, pkd.wh_id , pkd.order_number, pkd.wave_id, pkd.status, pkd.lot_number,wkq.work_q_id,wkq.work_status,pkd.pick_area,pkd.pendency
        FROM DBO.t_pick_detail pkd
        INNER JOIN DBO.t_work_q wkq ON wkq.work_q_id = pkd.work_q_id
        WHERE pkd.work_type IS NULL
        AND wkq.work_status <> 'C'
        AND pkd.status <> 'RELEASED'
        AND pkd.wh_id = '499'
        AND NOT EXISTS (SELECT 1 FROM DBO.t_pick_detail pkd2
                          WHERE pkd2.work_q_id = wkq.work_q_id
                          AND pkd2.wh_id = wkq.wh_id
                          AND pkd2.status = 'RELEASED')
        AND NOT EXISTS (SELECT 1 FROM DBO.t_work_q_assignment wka1
                          WHERE pkd.work_q_id = wka1.work_q_id
                          AND pkd.wh_id = wka1.wh_id)
        AND EXISTS (SELECT 1 FROM DBO.t_al_host_shipment_master spm
...
INSERT INTO dbo.t_exception_log
  ...
  SELECT DISTINCT bloco1.order_number,
                  'Informação Base - Script de Corte',
                  bloco1.item_number,
                  bloco1.wh_id,
                  bloco1.plan_qty,
                  bloco1.wave_id,
    ...
    FROM (SELECT pkd.wave_id,
            ...
            LEFT JOIN (SELECT sto.item_number,
                             sto.wh_id,
                             SUM(sto.actual_qty) AS qty_sto,
                             SUM(sto.unavailable_qty) AS qty_sto_una,
                             SUM(sto.actual_qty) - SUM(sto.unavailable_qty) AS dis
                        FROM DBO.t_stored_item sto
                        inner join DBO.t_location loc
                            on loc.wh_id = sto.wh_id
                            and loc.location_id = sto.location_id
                        inner join DBO.t_pick_area pka
                            on pka.wh_id = loc.wh_id
                            and pka.pick_area = loc.pick_area
                       WHERE sto.TYPE = 0
                       and sto.status = 'A'
                       and loc.status <> 'I'
                       GROUP BY sto.item_number, sto.wh_id) sto
    ...
    -- Se o pedido tiver alguma coisa já separada, tem que ser cortado pela operação
	AND NOT EXISTS (SELECT 1
					FROM DBO.t_tran_log LOG
     ...
     AND NOT EXISTS
    (SELECT 1 FROM DBO.t_stored_item sto WHERE sto.TYPE = bloco1.pick_id)
		AND NOT EXISTS (select 1 from DBO.t_exception_log log2
							where bloco1.order_number = log2.control_number
							and bloco1.item_number = log2.item_number
							and log2.tran_type = '033'
							and log2.lot_number = 'PRO_E');
...
-- FIM INSERCAO
    ...
    IF v_numCount = 0 THEN
            ...
            UPDATE DBO.t_exception_log set status = rec_main.tipo, suggested_value = 'PEND'
                WHERE control_number = rec_main.control_number
                and tran_type = '033';
        ...
        FOR rec_ord IN c_ord  LOOP
                ...
                -- SET BO_QTY
                UPDATE DBO.t_order_detail set bo_qty = rec_ord.quantity
                ...
                IF v_wave IS NULL THEN
                  UPDATE DBO.t_order_detail set afo_plan_qty = 0
                      WHERE order_number    = rec_ord.order_number
                      and wh_id             = rec_ord.wh_id
                      and item_number       = rec_ord.item;
                end if;
				...
				-- Adicionado o User Assigned
                UPDATE DBO.t_pick_detail 
						set status 		= 'PICKED'
						, lot_number 	= 'HJS'
						, pendency 		= 'C'
						, user_assigned	= 'HJS_CORTE'
                    WHERE order_number  = rec_ord.order_number
                    and wh_id           = rec_ord.wh_id
                    and status          = 'RELEASED'
                    and item_number     = rec_ord.item;
        ...
        v_numCount:= 0;
        -- Atualizar Status do PEDIDO na DBO.t_order
        SELECT COUNT(*) INTO v_numcount FROM DBO.t_order_detail
        WHERE order_number = rec_main.control_number
        AND bo_qty = 0;

        IF v_numCount > 0 THEN
            -- Se exister ainda Itens que não foram cortados (bo_qty = 0) Atualizar para D
            UPDATE DBO.t_order set status  = 'D' WHERE order_number = rec_main.control_number;
        ELSE
            --  Se não, seta pra C
            UPDATE DBO.t_order set status  = 'C' WHERE order_number = rec_main.control_number;
        END IF;


        ---============ VERIFICAR SE O ITEM CORTADO É O ÚLTIMO ITEM A SER DISTRIBUÍDO  ============---

        SELECT COUNT (*) INTO v_numPicked FROM DBO.t_pick_detail
        WHERE ORDER_NUMBER = rec_main.control_number
        AND WH_ID = rec_main.wh_id
        AND STATUS = 'PICKED'
        AND picked_quantity > 0;

        SELECT COUNT (*) INTO v_numReleased FROM DBO.t_pick_detail
        WHERE ORDER_NUMBER  = rec_main.control_number
        AND WH_ID           = rec_main.wh_id
        AND STATUS          = 'RELEASED';
        ...
        ELSE
            ...
            INSERT INTO DBO.v_al_host_shipment_master(
                host_group_id,
                transaction_code,
                order_number,
                ...
                'il11405'--:Printer:
                FROM
                DBO.t_pick_detail pkd,
                DBO.t_stored_item sto,
                DBO.t_order orm
                left join t_vendor vdo
                on orm.bill_to_code = vdo.vendor_code,
                v_type typ
                WHERE pkd.wh_id = sto.wh_id
                ...
                INSERT INTO v_al_host_shipment_detail(
                    ...
                    FROM DBO.t_sto_attrib_collection_detail tsacd
                    ...
                    DBO.v_al_host_shipment_master al_shm,
                    t_attribute_legacy_map alm,
                    DBO.t_item_master itm,
                    DBO.t_order orm
                    WHERE pkd.load_id = rec_main.control_number
						AND pkd.wh_id = rec_main.wh_id
...
COMMIT;
    ...
    -- FECHAMENTO DE TAREFAS
        ...
        UPDATE DBO.t_work_q set work_status = 'C' where work_q_id = rec_CloseTasks.WK_ID
        AND wh_id =  rec_CloseTasks.wh_id
        and work_status <> 'C';

        -- RASTREABILIDADE
        INSERT INTO DBO.t_tran_log_HOLDING(TRAN_LOG_HOLDING_ID, TRAN_TYPE, DESCRIPTION, START_TRAN_DATE, START_TRAN_TIME, EMPLOYEE_ID, WH_ID, CONTROL_NUMBER, CONTROL_NUMBER_2, LOT_NUMBER, LOCATION_ID, LOCATION_ID_2)
    ...
    FOR rec_main2 IN c_main2  LOOP
        ...
        END IF;
          ...
          UPDATE DBO.t_exception_log set suggested_value = 'OK'
        ...
        SELECT
            (SELECT sm.shipment_id FROM DBO.t_al_host_shipment_master sm
        ...
        FROM DBO.t_order_detail ord
        INNER JOIN DBO.t_order orm
        ON ord.wh_id = orm.wh_id
        AND ord.order_number = orm.order_number
        INNER JOIN DBO.t_item_master itm
        ...
        IF rec_main2.tipo = 'PARTIAL' THEN
            ...
            -- CHAMAR SERVIÇO
            SELECT DBO.PKG_WEBSERVICES.USF_CALL_WEBSERVICE('EXP_CANCEL_PARTIAL_ORDER', rec_main2.host_group) INTO v_vchReturn FROM DUAL;
            
            -- (PARCIAL - 361) - Inserir RASTREABILIDADE 
            ...
            SELECT
                NULL
                , '361'
                , (select description from DBO.t_transaction where tran_type = '361') -- DESCRIÇÃO
                , SYSDATE
                , SYSDATE
                , 'HJS_CORTE'
                , ord.order_number
            ...
            FROM DBO.t_order_detail ord
                ...
                INNER JOIN DBO.t_item_master itm
                    ...
                    AND itm.item_number = ord.item_number
                LEFT JOIN DBO.t_pick_detail pkd
                    ON PKD.order_number = ord.order_number
                    and PKD.wh_id = ord.wh_id
                    and PKD.item_number = ord.item_number
                WHERE ord.wh_id = rec_main2.wh_id
                AND ord.order_number = rec_main2.control_number
                AND ord.bo_qty > 0
                AND not exists (select 1 from DBO.t_tran_log log
                                where log.control_number = ord.order_number
                                and log.item_number = ord.item_number
                                and log.wh_id = ord.wh_id
                                and log.tran_type in ('360','361'));
        ...
        ELSIF rec_main2.tipo = 'COMPLETE' THEN
            ...
            SELECT
                NULL
                , '360'
                , (select description from DBO.t_transaction where tran_type = '360') -- DESCRIÇÃO
                , SYSDATE
                , SYSDATE
                , 'HJS_CORTE'
                , ord.order_number
            ...
            FROM DBO.t_order_detail ord
             ...
             SELECT DBO.PKG_WEBSERVICES.USF_CALL_WEBSERVICE('EXP_CANCEL_ORDER', rec_main2.host_group) INTO v_vchReturn FROM DUAL;
    ...
    select DBO.PKG_WEBSERVICES.USF_CALL_WEBSERVICE('EXP_PRE_SHIP', rec_NF.HOST_GROUP_ID) into v_vchReturnNF FROM DUAL;

    insert into DBO.t_tran_log_HOLDING(tran_log_holding_id, tran_type, description, start_tran_date, start_tran_time, employee_id, control_number, wh_id)
    values(null, '071', 'Interface PRE_SHIP (Corte)' , sysdate, sysdate, 'HJS', rec_nf.order_number, rec_nf.wh_id);

    UPDATE DBO.t_al_host_shipment_master set pro_number = 'P' where host_group_id = rec_NF.HOST_GROUP_ID and transaction_code = '936';

    --Atualizacao da t-order
    UPDATE DBO.t_order SET created_nf = 'Y' WHERE ORDER_NUMBER = rec_NF.ORDER_NUMBER;
